...
public class AlphaDepositBridgePayCallBackTestCases extends DepositTestCases {
	...
	public void initiEnv(String brand,String server, ITestContext context) {
		...
		Object data[][] = TestDataProvider.getAlphaCCallBackUsersData(brand, server);

		assertNotNull(data);
		openAPI = (String) data[0][5];
		adminData = TestDataProvider.getAlphaCCallBackUsersData(brand, server);
		adminPaymentAPI = new AdminAPIPayment((String) adminData[0][4], GlobalProperties.REGULATOR.valueOf((String)adminData[0][0]),(String)adminData[0][7],(String)adminData[0][8], BRAND.valueOf(brand.toUpperCase()), GlobalProperties.ENV.valueOf("ALPHA"));
