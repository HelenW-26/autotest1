...
public class UATAPIAdminPaymentTestCases extends AdminPaymentTestCases {
	...
	public void initEnv(String brand, String server, ITestContext context) {
		...
		ccData = UATTestDataProvider.getUATAPPRegUsersData(brand, server);
		lbtData = UATTestDataProvider.getUATLBTUsersData(brand);
		wdUserData = UATTestDataProvider.getUATAPIWithdrawUsersData(brand,server);
		transferData = UATTestDataProvider.getUATAPIAccTransferUsersData(brand,server);
	...
	public void testAPIAdminWithdrawalAudit(String brand) {
		try {
			...
			this.apiAdminWithdrawAuditPage(GlobalProperties.ENV.valueOf("UAT"), GlobalProperties.BRAND.valueOf(brand), GlobalProperties.REGULATOR.valueOf((String)adminData[0][0]));
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}
