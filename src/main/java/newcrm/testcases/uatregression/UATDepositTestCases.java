...
public class UATDepositTestCases extends DepositTestCases {
	...
	public void initiEnv(String brand,String server, ITestContext context) {
		...
		adminData = UATTestDataProvider.getUATRegUsersData(brand, server);
		adminPaymentAPI = new AdminAPIPayment((String) adminData[0][4], GlobalProperties.REGULATOR.valueOf((String)adminData[0][0]),(String)adminData[0][7],(String)adminData[0][8], BRAND.valueOf(brand.toUpperCase()), GlobalProperties.ENV.valueOf("UAT"));
		launchBrowser("uat","true",brand,(String)data[0][0],(String)data[0][3],(String)data[0][1],(String)data[0][2],"","","","True",context);
		WebDriver tempDriver = (WebDriver) context.getAttribute("driver");
		if (tempDriver != null) {
			this.driver = tempDriver;
			context.setAttribute("driver", this.driver);
			LogUtils.info("BeforeClass 中 driver 初始化成功");
		} else {
			LogUtils.error("BeforeClass 中 driver 初始化失败", null);
		}
	}
	...
	public void testDeposit() {
		...
		try {
			...
			if("mo".equalsIgnoreCase(Brand)) {
				...
				MethodTracker.trackMethodExecution(this, "testCryptoUSDCERCNew", true, null,driver);
			...
			{
//				MethodTracker.trackMethodExecution(this, "testInterBankTransPayNew", true, null);
				MethodTracker.trackMethodExecution(this, "testCryptoERCNew", true, null,driver);
//				MethodTracker.trackMethodExecution(this, "testBridgePayNew", true, null);
			}
			else{
