...
public class CPNewWithdraw {
	...
	void WithdrawNewI18NBank(String TraderURL,String Brand, String TestEnv) throws Exception
	{


		String fromAccount = "",trans_type="";
		String methodName="",actualAmount="";
		boolean cc_only_flag = false;
		...
		for (WebElement webElement : fromElement)
		{
		   ...
		   if(Utils.funcCheckAccoutGroup(Brand, TestEnv, fromAccount) && amount.compareTo(miniAmount)>=0)		   {
			     ...
			     //Click Continue
			     i18nCls.getContinueBtn().click();
			     
			     /*If I18N withdraw method can be found, perform I18N withdraw; 
			     * if not, perform CC withdraw.
 			     */
			     try {
				     ...
				     actualAmount = i18nCls.getAvailableAmount().getText();
				     
			     }catch (Exception e) {
			    	 
			    	 cc_only_flag = true;
			    	 System.out.println("Not able to see International bank wire trasfer withdraw method.");
			     }
				...
				if (cc_only_flag == true) {
					if(Brand.equalsIgnoreCase("vt"))
					{
						trans_type = "Credit/Debit card";
					}else
					{
						trans_type = "Credit/Debit Card";
					}
				...
				CPDeposit.funcValidateTransacHistory(driver, fromAccount, trans_type, actualAmount, Brand);		
	...
	void funcNavigate2WithdrawHistory(String Brand) throws Exception
	{
		...
		Utils.waitUntilLoaded(driver);
		driver.findElement(By.xpath("//ul[@class='tab_demo']//li[contains(translate(text(),' Withdrawls',' WITHDRAWALS'),' WITHDRAWALS')]")).click();
	...
	void withdrawI18NForm(withdrawI18N i18nCls) throws Exception
	{
		...
		}else if(bankAcctList.size()>1)
		{
			...
			*/
			...
			/* 16/06/2021 by Alex Liu
			 * Temporary commented the below lines due to duplicate web elements
			 */ 
